...
# ./prop /projects/amp/peptaid/anura/apetersi/skin_liver/homology/jackhmmer.faa | grep -E "^  [0-9]+| \*ProP\*" | grep '\*ProP\*' -B1 --no-group-separator | awk -v var="$outdir" 'BEGIN{x="/dev/null"}/^  [0-9]+/{x=var"/F"++i".txt";}{print > x;}
function get_help() {
...
if [[ -f $outdir/CLEAVE.DONE ]]
then
	rm $outdir/CLEAVE.DONE
fi

if [[ -f $outdir/CLEAVE.FAIL ]]
then
	rm $outdir/CLEAVE.DONE
fi
...
then
	...
	if [[ "$permissions" != "drwxrw[sx]rwt"  && "$owner" == "$(whoami)" ]]
	then
...
if command -v $RUN_SIGNALP &> /dev/null
then
	signalp=true
	signalp_opt="-s"
