...
charge=2
while getopts :a:c:hl:o:s:t: opt
do
	case $opt in
...
if [[ -f $outdir/AMPLIFY.DONE ]]
then
	rm $outdir/AMPLIFY.DONE
fi
...
then
	if [[ ! -f $input ]]
	then
		echo "Input FASTA file does not exist!" 1>&2
	else
		echo "Input FASTA file is empty!" 1>&2
	fi
	exit 2
fi
...
if [[ ! -f $outdir/AMPlify_results.txt ]]
then
...
# convert TXT to TSV
if [[ -s $outdir/AMPlify_results.faa ]]
then
...
### Filter all sequences for those that are labelled AMP
#----------------------------------------------------------
...
if [[ -s ${outfile} || $(grep -c '^>' $outfile) -gt 1 ]]
then
...
### Filter all sequences for those with AMPlify score >= $confidence
#--------------------------------------------------------------------
...
if [[ -s ${outfile_conf} || $(grep -c '^>' $outfile_conf) -gt 1 ]]
then
	echo "Removing duplicate sequences..." 1>&2
...
### Filter all sequences for those labelled 'AMP' and length <= $length
#--------------------------------------------------------------------
...
if [[ -s ${outfile_short} || $(grep -c '^>' ${outfile_short}) -gt 1 ]]
then
	echo "Removing duplicate sequences..." 1>&2
...
if [[ -s $outfile_charge || $(grep -c '^>' $outfile_charge) -gt 1 ]]
then
	echo "Removing duplicate sequences..." 1>&2
...
### Filter all sequences for those with AMPlify score >= $confidence and length <= $length
#--------------------------------------------------------------------
...
if [[ -s $outfile_conf_short || $(grep -c '^>' $outfile_conf_short) -gt 1 ]]
then
	echo "Removing duplicate sequences..." 1>&2
...
### Filter short and confident sequences for those with AMPlify score >= $confidence and length <= $length, and charge >= $charge
#--------------------------------------------------------------------
...
if [[ -s $outfile_conf_short_charge || $(grep -c '^>' $outfile_conf_short_charge) -gt 1 ]]
then
	echo "Removing duplicate sequences..." 1>&2
