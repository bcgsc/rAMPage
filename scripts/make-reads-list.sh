...
# if there is only one field, do not pool
	...
	do
		...
		if [[ "$num_unique" -gt 1 && "$num_unique" -lt "$num_rows" ]]
		then 
			indices+=( $j )
		# if every entry is unique in the column, store it in equal array
		elif [[ "$num_unique" -eq "$num_rows" ]]
		then
			equal+=( $j )
		fi
	done 
	...
	then
		...
		then
			filtered=()
			for x in $(seq 0 $(( ${#indices[@]}-2 )) )
			do
				same=false
				last=false
				for y in $(seq $(( x+1 )) $(( ${#indices[@]}-1 )) )
				do
