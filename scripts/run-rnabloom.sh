...
function get_help() { # DESCRIPTION
	...
	# reads list
	echo "EXAMPLE READS LIST (NONSTRANDED):" 1>&2
	echo -e "\
	...
	echo "EXAMPLE READS LIST (STRANDED):" 1>&2
	echo -e "\
...
memory_bool=false
email=false
while getopts :a:hm:o:t: opt
do
	case $opt in 
		a) address="$OPTARG";email=true;;
		h) get_help;;
		m) memory_bool=true; memory="-Xmx${OPTARG}";;
...
echo "START: $(date)" 1>&2
start_sec=$(date '+%s')
echo -e "Creating output directory: $outdir\n" 1>&2
mkdir -p $outdir
...
then
	...
	echo "ERROR: Cannot find minimap2 in your PATH!" 1>&2; printf '0.%s=' $(seq $(tput cols)) 1>&2; echo 1>&2;
	get_help
fi
...
then
	...
	echo "ERROR: Cannot find ntCard in your PATH!" 1>&2; printf '0.%s=' $(seq $(tput cols)) 1>&2; echo 1>&2;
	get_help
fi
...
then
	...
	echo "ERROR: Cannot find JRE 8 in your PATH!" 1>&2; printf '0.%s=' $(seq $(tput cols)) 1>&2; echo 1>&2;
	get_help
fi
...
if [[ "$java_version" != 1.8* ]]
then
	echo "ERROR: RNA-Bloom requires Java SE Runtime Environment (JRE) 8." 1>&2
	echo "Java version detected: $java_version" 1>&2; printf '%.0s=' $(seq 1 $(tput cols)) 1>&2; echo 1>&2
	get_help
fi
...
then
	...
	then
		stranded=true
	elif [[ -f $workdir/NONSTRANDED.LIB ]]
	then
		stranded=false
	else
...
then
	...
	mergepool_opt=""
	pool=false
elif [[ "$(awk '{print $1}' $readslist | sort -u | wc -l)" -eq 1 ]]
then
