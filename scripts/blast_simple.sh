...
function get_help() {
	{
		...
		echo "DESCRIPTION:"
		echo -e "\
		\tCharacterizes the novelty of AMPs using BLASTp. Does not join with AMPlify results.\n \
		\n \
		\tOUTPUT:\n \
		\t-------\n \
		...
		echo "USAGE(S):"
		echo -e "\
        \t$PROGRAM [-o <outdir>] [-t <int>]  -d <preformatted BLAST nr database> <AMP FASTA file>\n \
		" | table
