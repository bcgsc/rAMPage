...
$(ROOT_DIR)/amps.nr.faa: $(FASTA) $(TSV) $(ROOT_DIR)/scripts/run-cdhit.sh $(ROOT_DIR)/scripts/get-runtime.sh
	cat $(FASTA) > $(ROOT_DIR)/amps.faa
	$(ROOT_DIR)/scripts/run-cdhit.sh -v $(ROOT_DIR)/amps.faa &> $(ROOT_DIR)/logs/11-redundancy_removal.log
	cat $(TSV) > $(ROOT_DIR)/AMPlify_results.tsv # need to remove headers

# run sable
sable: $(ROOT_DIR)/sable_output.tsv

$(ROOT_DIR)/sable/OUT_SABLE_graph: $(ROOT_DIR)/amps.nr.faa $(ROOT_DIR)/scripts/run-sable.sh
	$(ROOT_DIR)/scripts/run-sable.sh -o sable $(ROOT_DIR)/amps.nr.faa &> $(ROOT_DIR)/logs/12-sable.log

$(ROOT_DIR)/sable/sable_output.tsv: $(ROOT_DIR)/sable/OUT_SABLE_graph $(ROOT_DIR)/scripts/process-sable.sh
	$(ROOT_DIR)/scripts/process-sable.sh $(ROOT_DIR)/sable/OUT_SABLE_graph
